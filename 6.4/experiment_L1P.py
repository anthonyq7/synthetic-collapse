...
SYSTEM_PROMPT = f"""
    You are a student writing about a topic using a provided set of articles.
...
async def generate_gpt(prompt_messages: List[Dict], paper_id: str, paper_data: Dict):
    async with openai_semaphore:
        ...
        if not content:
            print(f"Empty response {paper_id}. Retrying with higher token budget...")
...
async def generate_paper(prompt_messages: List[Dict], paper_id: str, paper_data: Dict, model_tag: str):
    try:
        ...
        print(f"[{paper_id}] generate_paper exhausted retries: {exc}")
...
async def generate_node(node: int) -> Dict:
    ...
    hydrated_by_id: Dict[str, List[Dict]] = {}
    ...
    with prompts_path.open("w") as prompts_file:
        for record in records:
            ...
            hydrated_by_id[record["id"]] = hydrated_candidates
