...
class PaperResponse(BaseModel):
    ...
    body: str = Field(description="support claims by citing relevant articles inline using parenthetical citation format, e.g. ([Surname], [Year])")
...
def generate_prompts(node: int):
    ...
    with open(f"gemini/prompts/N_{node}_inputs.jsonl", "w") as f:
    ...
    async with semaphore:
        try:
            ...
            if response.usage_metadata:
                ...
                usage["completion_tokens"] += (response.usage_metadata.candidates_token_count + (response.usage_metadata.thoughts_token_count or 0))
                usage["total_tokens"] += response.usage_metadata.total_token_count

            content = response.text
            finish_reason = response.candidates[0].finish_reason

            if not content or finish_reason.name == "MAX_TOKENS":
...
async def generate_node(node: int):
    ...
    with open(f"gemini/prompts/N_{node}_inputs.jsonl", "r") as f:
        for raw_line in f:
    ...
    with open(f"gemini/output/node_{node}/node_{node}_stats.jsonl", "w") as f:
    ...
    with open(f"gemini/output/citation_counts/hallucinations.jsonl", "a") as f:
        for item in hallucinations:
            f.write(json.dumps(item) + "\n")
        f.flush()

    with open(f"gemini/output/citation_counts/over_cap_violations.jsonl", "a") as f:
        for item in over_cap_violations:
            f.write(json.dumps(item) + "\n")
        f.flush()

    with open(f"gemini/output/node_{node}/node_{node}_token_usage.jsonl", "w") as f:
    ...
    with open(f"gemini/output/node_{node}/node_{node}_token_totals.jsonl", "w") as f:
...
async def run_experiment():
    ...
    random.seed(SEED)
    os.makedirs("gemini/output/seed", exist_ok=True)
    os.makedirs("gemini/output/master", exist_ok=True)
    os.makedirs("gemini/output/citation_counts", exist_ok=True)
    os.makedirs("gemini/prompts", exist_ok=True)
    open("gemini/output/citation_counts/hallucinations.jsonl", "w").close()
    open("gemini/output/citation_counts/over_cap_violations.jsonl", "w").close()
    POOLED_PAPERS.extend(create_seed())

    for i in range(TOTAL_NODES):
        os.makedirs(f"gemini/output/node_{i}", exist_ok=True)
    ...
    with open("gemini/output/citation_counts/citation_counts.jsonl", "w") as f:
        for k, v in CITATION_COUNTS.items():
            f.write(json.dumps({k: v}) + "\n")
    
    with open("gemini/output/master/kv_pairs.jsonl", "w") as f:
        for (author, year), paper_id in SEEN_AUTHOR_YEAR_PAIRS.items():
            record = {"author": author, "year": year, "id": paper_id}
            f.write(json.dumps(record) + "\n")

    with open("gemini/output/master/token_usage.jsonl", "w") as f:
